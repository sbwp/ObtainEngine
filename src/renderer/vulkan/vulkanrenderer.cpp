...
namespace Obtain::Graphics::Vulkan {
	...
	void VulkanRenderer::createSwapChain() {
		...
		// Get number of images and resize before getting images
		// (because it can make more than we asked for)
	...
	void VulkanRenderer::recreateSwapChain() {
		...
		// If window is minimized or otherwise has 0 height or width,
	...
	void VulkanRenderer::createDepthResources() {
		...
		createImage(
			swapChainExtent.width, swapChainExtent.height, 1, msaaSamples, depthFormat, VK_IMAGE_TILING_OPTIMAL,
	...
	void VulkanRenderer::createTextureImage() {
		...
		transitionImageLayout(
			textureImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_UNDEFINED,
	...
	void VulkanRenderer::transitionImageLayout(
		...
		VkImageLayout newLayout, uint32_t mipLvls
	) {
		VkCommandBuffer commandBuffer = beginSingleTimeCommands();
	...
	void VulkanRenderer::createTextureImageView() {
		textureImageView = 
		createImageView(textureImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
	}

	VkImageView VulkanRenderer::createImageView(
		VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLvls
	) {
	...
	void VulkanRenderer::createTextureSampler() {
		...
		//samplerInfo.minLod = 0; // (default is 0)
		samplerInfo.maxLod = static_cast<float>(mipLevels);
		//samplerInfo.mipLodBias = 0; // (default is 0)
	...
	void VulkanRenderer::loadModel() {
		...
		for (const auto& shape : shapes) {
			for (const auto& index : shape.mesh.indices) {
				...
				// OBJ uses bottom left as origin for texCoords, so we have to flip Y
				vertex.texCoord = {
	...
	void VulkanRenderer::updateIndices() {
		VkDeviceSize bufferSize = sizeof(indices[0]) * indices.size();
	...
	void VulkanRenderer::drawFrame() {
		...
		// If we must, recreate swapchain and abort. If it's just suboptimal,
		// let it go for now and we'll rebuild after this frame is presented.
		switch (result) {
		...
		// Now rebuild even if it's just suboptimal.
		switch (result) {
			case VK_ERROR_OUT_OF_DATE_KHR:
			case VK_SUBOPTIMAL_KHR:
				recreateSwapChain();
	...
	void VulkanRenderer::updateUniformBuffer(uint32_t currentImage) {
		...
		// Flip Y axis on projection matrix since GLM uses OpenGL's positive-up Y axis
		ubo.proj[1][1] *= -1;
